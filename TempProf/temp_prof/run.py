...
def analyze_LCs(lightcurves, times, errors, lc_lengths, N_tp, yvals, AGN_params, filters, lambda_vals, map_fname, anim_fname,
                ...
                interval=50, fps=.25, cmap_num=16, interpolation='antialiased'):
    ...
    for i in tqdm(  range(len(filters))  ):
        ...
        Fnu = 10**( (lc+48.6)/(-2.5) ) * nu / lambda_vals[i]
        Fnu_err = lc_err * (-.4*np.log(10))* 10**( -2*( lc + 243/5 )/5 ) * nu / lambda_vals[i]
    ...
    plot_profs_out(dToT_outputs_reshape, tp_vals*(1+z) + min_td-50000, yvals, xi_vals, chi2_mean, 
                   fname=map_fname, show=True, cmap_num=cmap_num, interval=interval, interpolation=interpolation)
